...
public class Aggregate extends Expression {
     ...
     * -------------------------------------------------
     */
    static final int KMEANS = 17;
    
    static final int FIRST=18;
    ...
    public Value getValue(Session session) {
        if (select.isQuickAggregateQuery()) {
            ...
            case LAST:
            case FIRST:
			    boolean firstf= type==FIRST;
            	Index indexf=getColumnIndex();
            	Cursor cursorf = indexf.findFirstOrLast(session, firstf );
                SearchRow rowf = cursorf.getSearchRow();
                Value vf;
                if (rowf == null) {
                    vf = ValueNull.INSTANCE;
                } else {
                    vf = rowf.getValue(indexf.getColumns()[0].getColumnId());
                }
                return vf;
            case KMEANS:
            	Table table1 = select.getTopTableFilter().getTable();
                return ValueLong.get(table1.getRowCount(session));
            	 /**
                 * -------------------------------------------------
                 */
            default:
